...
## Interface-OOP · Log 11

### 🧪 今日成果
- 更新 `UseAbilities()`：如果技能不可用，自动回退为普通攻击
- 构建 1v1 回合战斗系统：`StartBattle()` 控制战斗循环、胜负判断清晰
- 战斗记录完整写入 `BattleLog`，便于后续调试与展示

### ❓ 当前疑问
- 当前技能选择策略为“找到第一个可用技能即使用”，是否可扩展为更智能的决策方式？

### 🔜 明日预定
- 增加主动攻击或技能选择策略（如判断敌人血量、角色状态等）
- 尝试将战斗日志导出为 Markdown 或 JSON 文件